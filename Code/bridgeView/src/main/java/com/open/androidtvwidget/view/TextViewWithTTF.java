...
public class TextViewWithTTF extends TextView {
    ...
    public TextViewWithTTF(Context context, AttributeSet attrs) {
        ...
        setTypeface(SFontMgr.getTypeface(context, array.getString(R.styleable.TextViewWithTTF_ttf_name)));
        array.recycle();
    }
    ...
    public TextViewWithTTF(Context context, String aTTFName) {
        super(context);
        setTypeface(SFontMgr.getTypeface(context, aTTFName));
    }

    public void setFont(Context context, String aTTFName) {
        setTypeface(SFontMgr.getTypeface(context, aTTFName));
    }
}
...
class TypeFaceMgr {
    ...
    public Typeface getTypeface(Context context, String aTTFName) {
        if (mTypefaces.containsKey(aTTFName)) {
            return mTypefaces.get(aTTFName);
        } else {
            Typeface font;
            try {
                font = Typeface.createFromAsset(context.getAssets(), "fonts/" + aTTFName);
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
